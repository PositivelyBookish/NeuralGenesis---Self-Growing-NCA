...
class NCA(nn.Module):
    def __init__(self, n_channels=16, hidden_channels=128, filter="sobel", fire_rate=0.5, device=None):
        ...
        # update
        self.rnn = nn.RNN(input_size=3 * n_channels, hidden_size=hidden_channels, batch_first=True)
        self.conv1 = nn.Conv2d(hidden_channels, hidden_channels, kernel_size=1)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv2d(hidden_channels, n_channels, kernel_size=1, bias=False)
    ...
    def update(self, x):
        ...
        y = self.perceive(x)  # Explicitly perceive
        y = y.view(y.shape[0], y.shape[1], -1).permute(0, 2, 1)  
        rnn_out, _ = self.rnn(y)  
        rnn_out = rnn_out.permute(0, 2, 1).view(y.shape[0], -1, x.shape[2], x.shape[3])  
        dx = self.conv2(self.relu(self.conv1(rnn_out)))  
