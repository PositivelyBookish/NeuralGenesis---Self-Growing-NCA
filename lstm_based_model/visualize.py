...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--config", type=str, default="/home/Souvik.Roy/lstm_nca/visualize_config.yaml", help="Path to config file.")
    ...
    # load model
    model = NCA_LSTM(n_channels=16, filter=config["filter"], device=torch.device("cpu"))
    ...
    hidden = (
    	torch.zeros(1, 1, 64), # Hidden state # store frames for animation
    	torch.zeros(1, 1, 64) # Cell state frames = []
    )
    # run model
    for i in tqdm(range(config["iterations"])):
        cs, hidden = model(cs, hidden)
